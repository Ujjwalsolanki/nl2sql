...
try:
    ...
    logger = setup_logging(log_file="nl2sql_app.log", log_level="INFO")
except ImportError:
...
if __name__ == "__main__":
    logger.info("--- Starting NL2SQLChainManager Example (with History) ---")

    try:
        # 1. Initialize DatabaseManager to get the Langchain SQLDatabase object
        logger.info("Initializing DatabaseManager...")
        db_manager = DatabaseManager()
        langchain_sql_db = db_manager.get_langchain_db()
        logger.info("DatabaseManager initialized and Langchain SQLDatabase obtained.")

        # 2. Initialize NL2SQLChainManager with the Langchain SQLDatabase
        logger.info("Initializing NL2SQLChainManager...")
        nl2sql_chain = NL2SQLChainManager(langchain_sql_db)
        logger.info("NL2SQLChainManager initialized.")

        # Initialize chat history (empty at the start)
        conversation_history = []

        # First question
        query_1 = "how many orders are there?"
        logger.info(f"\nUser: {query_1}")
        response_1 = nl2sql_chain.process_query(query_1, conversation_history)
        logger.info(f"AI: {response_1}")
        # Update history
        conversation_history.append(HumanMessage(content=query_1))
        conversation_history.append(AIMessage(content=response_1))

        # Second question, referring to context
        query_2 = "how many disputed?" # This should refer to orders
        logger.info(f"\nUser: {query_2}")
        response_2 = nl2sql_chain.process_query(query_2, conversation_history)
        logger.info(f"AI: {response_2}")
        # Update history
        conversation_history.append(HumanMessage(content=query_2))
        conversation_history.append(AIMessage(content=response_2))

        # Another example to test history
        query_3 = "What is the credit limit for the customer 'Atelier graphique'?"
        logger.info(f"\nUser: {query_3}")
        response_3 = nl2sql_chain.process_query(query_3, conversation_history)
        logger.info(f"AI: {response_3}")
        conversation_history.append(HumanMessage(content=query_3))
        conversation_history.append(AIMessage(content=response_3))

        query_4 = "And what about 'Signal Gift Stores'?"
        logger.info(f"\nUser: {query_4}")
        response_4 = nl2sql_chain.process_query(query_4, conversation_history)
        logger.info(f"AI: {response_4}")
        conversation_history.append(HumanMessage(content=query_4))
        conversation_history.append(AIMessage(content=response_4))


    except ValueError as ve:
        logger.critical(f"Configuration Error: {ve}")
    except ConnectionError as ce:
        logger.critical(f"Database Connection Error: {ce}")
    except Exception as e:
        logger.critical(f"An unexpected error occurred in main execution: {e}", exc_info=True)

    logger.info("--- NL2SQLChainManager Example Finished ---")
